...
def run_on_folders(parent_folder: str) -> None:
    ...
    for entry in os.scandir(parent_folder):
        if entry.is_dir():
            ...
            root = os.path.join(entry.path, "suite2p\\plane0\\") # Path to a single Suite2p plane folder
            ...
            dff = np.memmap(os.path.join(root, f'{prefix}dff.memmap.float32'), dtype='float32', mode='r',
                            shape=(T, nROIs))
...
if __name__ == "__main__":
    logfile = open("raster_and_heatmaps_plots.log", "a")
