"""
cluster_median_dff_gui.py - Standalone GUI.
...
cluster from the dropdown, preview it in the Fig7_1 style, and on Calculate save:
  - <cluster>_median_dff.png   : median dF/F trace over time
  - <cluster>_median_dff.csv   : time_s, median_dff columns
...
class ClusterMedianDffGui(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Cluster median dF/F")
    ...
    def _do_calculate(self, labels: list[str]) -> None:
        ...
        median_dff = np.median(traces, axis=1)
        ...
        png_path = out_dir / f"{combo_label}_median_dff.png"
        csv_path = out_dir / f"{combo_label}_median_dff.csv"

        with open(csv_path, "w", newline="") as fh:
            writer = csv.writer(fh)
            writer.writerow(["time_s", "median_dff"])
            writer.writerows(zip(time_s.tolist(), median_dff.tolist()))

        fig = Figure(figsize=(10, 3.5), dpi=150)
        ax = fig.add_subplot(1, 1, 1)
        ax.plot(time_s, median_dff, color="#1f77b4", linewidth=0.8)
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Median ΔF/F")
        ax.set_title(
            f"{combo_label} median ΔF/F  (n={rois.size} ROIs, fps={self.fps:g})"
        )
        ax.margins(x=0)
        fig.tight_layout()
