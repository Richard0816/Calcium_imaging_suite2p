...
def _zscore_cols(X, eps=1e-12):
    """Z-score each column of X. Columns with zero std become zeros."""
...
def run_cluster_xcorr_full_fast(
    ...
    *,
    ...
    clusters = _load_clusters_from_dir(base_dir)
    if len(clusters) < 1:
...
def run_cluster_xcorr_per_event_fast(
    ...
    *,
    ...
    gpu_ok = bool(use_gpu and cp is not None)
    cluster_names = sorted(clusters.keys())
    cluster_rois_int = {n: np.asarray(clusters[n], dtype=np.int64)
    ...
    for ev_idx, (s_sec, e_sec) in enumerate(event_windows):
        ...
        cluster_Z_ev = {}
        for n in cluster_names:
            X = np.asarray(clip[:, cluster_rois_int[n]],
        ...
        for iA, cA in enumerate(cluster_names):
            for cB in cluster_names[iA:]:
                roisA = cluster_rois_int[cA]
                roisB = cluster_rois_int[cB]
